...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		/*This method performs addition by adding the elements (respective to the current index)
		 * of first and second linked list. It saves the last digit of this result in the sum linked list
		 * and sets the first element of the result as the carry which will be added in the next iteration.
		 */
		int carry = 0;
		for(int i =0; i<secondClone.coefficients.size(); i++) {
			int tmp = firstClone.coefficients.get(i)+secondClone.coefficients.get(i)+carry;
			carry = tmp/base;
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		/*The product linked list is initialized to zero as the result from 
		 * the loop will be added to previous element of the product.
		 * The times method uses the helper method "timesSingleDigit" which multiplies each
		 * element of multiplicand with the element of multiplier (this) respective to the index.
		 * It then adds that product to the previous value of the product.
		 * It appends zeros (respective to the current index) to the end of the first product by calling timesBaseToThePower
		 */


		product.coefficients.addFirst(0);
		for (int i=0;i<multiplicand.coefficients.size();i++) {
			NaturalNumber tmp=timesSingleDigit(multiplicand.coefficients.get(i));
			product = product.plus(tmp.timesBaseToThePower(i)); //appends zeros according to the current index
		}
		/*
		 *           multiplicand
		 *          x  multiplier  (this)
	...
	/*product is an empty list of coefficients
	 * The helper method multiplies any given integer with the "this".
	 * It is based on the logic used for plus where it stores the last digit of the product in the product linked list
	 * and the first digit is taken as the carry.
	 */

	public NaturalNumber timesSingleDigit(int k){
		NaturalNumber product = new NaturalNumber(this.base);
		int carry = 0;
		for (int j=0; j<this.coefficients.size(); j++ ) {
			int tmp = (this.coefficients.get(j)*k) + carry;
			carry = tmp/this.base;
			product.coefficients.addLast(tmp%this.base);
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		{
			/*This if branch takes care of the borrow
			 * the condition is true only when the first element is < second element respective to the current index;
			 * if the element is zero the if branch sets it to "base -1" otherwise it simply subtracts one from the element
			 * and sets the condition to false
			 * The next if branch executes 
			 */
			if (borrow) {
				if (first.coefficients.get(i)==0) {
					first.coefficients.set(i, base-1);
				}
				else {
			...
			/*This if branch compares each elements of first linked list with the elements of the secondClone's linked list
			 * if the condition is valid it sets the borrow as true and adds the base to the element of 
			 * the first linked list 
			 * it then subtracts that value from the respective element of the smaller number 
			 */
			if (first.coefficients.get(i) < secondClone.coefficients.get(i)){
				borrow = true;
				tmp = (first.coefficients.get(i)+base)-secondClone.coefficients.get(i);

			}
			/*if the element of the first linked list is > than the element of the second linked list it simply 
			 * subtracts that element from the respective element of second linked list
			 */
			else {
				tmp = first.coefficients.get(i)-secondClone.coefficients.get(i);
			}
			difference.coefficients.addLast(tmp); //stored the value from the preceding if-else branch in the differenc linked list
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		//compares the dividend with the divisor ; if this (dividend) < divisor simply returns 0 as the quotient

		if (this.compareTo(divisor) < 0)	{
			quotient.coefficients.addFirst(0);
			return quotient;
		}
		/*This method takes a chunk of the dividend when this > divisor 
		 * and uses it to get the first quotient via the chunkDivison helper method
		 * the chunk starts from the most significant number of "this" (dividend)
		 * it makes the next chunk with the remainder left from the preceding division 
		 * and the next element in "this"
		 */

		int count = this.coefficients.size();//starts the chunk from the most significant digit 
		boolean firstChunk = true;

		NaturalNumber chunk = new NaturalNumber (base);

		while (count>0) {

			int countDigits = 0;
			while(chunk.compareTo(divisor)<0 && count>0) {
				int nextValue = this.coefficients.get(count-1);
				if(nextValue != 0 || chunk.coefficients.size()>0) {        	
					chunk.coefficients.addFirst(nextValue);
				}

				countDigits++; //keeps track of any zeros within the dividend
				count--;

			}
			//adds zero(s) in the quotient if there are zeros within the dividend
			if(firstChunk) {
				firstChunk = false;
			}
			else {
			...
			//adds zero in the quotient if the last chunk or the remainder is < divisor
			if(chunk.compareTo(divisor)<0 && count == 0) {
				quotient.coefficients.addFirst(0);

			}
			//uses chunkDivision to obtain the quotient and subtracts the result from the chunk
			else {
				int tmp = divisor.chunkDivision(chunk);
				quotient.coefficients.addFirst(tmp);
				chunk=chunk.minus(divisor.timesSingleDigit(tmp));
				chunk.removeLeadingZeros(); //removes any leading zeros from the chunk formed
	...
	/*This helper method returns quotient by comparing this with the chunk.
	 * It uses timesSingleDigit that multiplies this with i and returns i-1 when the product >= chunk.
	 * i-1 is the quotient
	 */

	public int chunkDivision (NaturalNumber chunk) {
		// This method assumes chunk >= this
		int tmp =1;
		while (tmp < base && timesSingleDigit(tmp+1).compareTo(chunk) <= 0){
	...
	//This helper method removes any leading zeros from the value (chunk) given to it
